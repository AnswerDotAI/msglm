...
# %% auto 0
__all__ = ['PROVIDER_MSG', 'mk_msg_openai', 'mk_msgs_openai', 'mk_msg_azure', 'mk_msgs_azure', 'Msg', 'OpenAiMsg', 'AnthropicMsg',
           'mk_msg', 'mk_msgs', 'mk_msg_anthropic', 'mk_msgs_anthropic', 'mk_ant_doc']
...
_OPENAI_SCHEMAS = {
    "responses": {
        ...
        "img_field": "image_url",
        "wrap_img_url": lambda url: url,
    },
    "chat": {
        "text_type": "text",
        "img_type": "image_url",
        "img_field": "image_url",
        "wrap_img_url": lambda url: {"url": url},
    },
}

# %% ../nbs/00_core.ipynb
class OpenAiMsg(Msg):
    "Helper class to create a message for the OpenAI and Azure/OpenAI APIs."
    def __init__(self, schema='responses'): self.schema = _OPENAI_SCHEMAS[schema]
...
def img_msg(self:OpenAiMsg, data:bytes)->dict:
    ...
    url = f"data:{mtype};base64,{img}"
    return {"type": self.schema["img_type"], self.schema["img_field"]: self.schema["wrap_img_url"](url)}

@patch
def text_msg(self:OpenAiMsg, s:str, text_only=False)->dict:
    "Convert `s` to a text message"
    if not s.strip(): s='.'
    if text_only: return s
    return {"type": self.schema["text_type"], "text":s}
...
# %% ../nbs/00_core.ipynb
PROVIDER_MSG = {
    'openai': partial(OpenAiMsg, schema='responses'),
    'azure': partial(OpenAiMsg, schema='chat'),
...
def find_block(self:OpenAiMsg, r):
    ...
    if hasattr(r, "choices"): return r.choices[0].message
    return r
